...
contract Liquidator is IUniswapV2Callee, Ownable {
    ...
    function uniswapV2Call(address sender, uint256 amount0, uint256 amount1, bytes calldata data) external override {
        ...
        IWETH(callback.tokenOut).deposit{ value: callback.amountOut }();
        // 7. repay WETH to uniswap pool
        address pairAddress = IUniswapV2Factory(_UNISWAP_FACTORY).getPair(callback.tokenIn, callback.tokenOut);
        IWETH(callback.tokenOut).transfer(pairAddress, callback.amountOut);
    ...
    function liquidate(address[] calldata path, uint256 amountOut) external {
        ...
        address pairAddress = IUniswapV2Factory(_UNISWAP_FACTORY).getPair(path[0], path[1]);
        ...
        data.tokenOut = path[0]; // eth
        data.amountIn = amountOut; // u
        data.amountOut = repayAmount; // eth

        IUniswapV2Pair(pairAddress).swap(0, amountOut, address(this), abi.encode(data));
    }
